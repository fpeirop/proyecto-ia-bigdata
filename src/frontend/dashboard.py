...
def fetch_api(endpoint: str, force_reload: bool = False) -> list | dict | None:
...
def registrar_historial(nombre_calle: str, intensidad: float, estado: str):
    ...
    st.session_state.history = pd.concat(
        [st.session_state.history, pd.DataFrame([nuevo_dato])], ignore_index=True
    )
...
def obtener_centro_y_zoom(calle_sel: str, display_df: pd.DataFrame) -> tuple[dict, float]:
    ...
    return {
        "lat": display_df.iloc[0]["latitude"],
        "lon": display_df.iloc[0]["longitude"],
    }, ZOOM_DETALLE
...
def renderizar_mapa(display_df: pd.DataFrame, center: dict, zoom: float) -> None:
    fig_map = px.scatter_mapbox(
        ...
        hover_data={
            ...
            "longitude": False,
            "status": True,
            "traffic_intensity": True,
        },
        center=center,
        zoom=zoom,
        height=450,  # Reducido ligeramente para ahorrar espacio
    )
...
with st.sidebar:
    ...
    filtro_estado = st.multiselect(
        "Filtrar por estado:", list(COLOR_ESTADO.keys()), default=list(COLOR_ESTADO.keys())
    )
    min_intensidad = st.slider("Intensidad mínima (veh/h):", 0, 5000, 0)
    st.markdown("---")
    if st.button("🗑️ Limpiar Historial"):
        st.session_state.history = pd.DataFrame(
            columns=["timestamp", "street", "intensity", "status"]
        )
        st.rerun()
...
if map_data_raw:
    ...
    df_filtered = df_map[
        df_map["status"].isin(filtro_estado) & (df_map["traffic_intensity"] >= min_intensidad)
    ]
    ...
    with col_sel:
        ...
        calle_sel = st.selectbox(
            "📍 Selecciona una ubicación o calle:", opciones, label_visibility="collapsed"
        )
    with col_btn:
    ...
    if calle_sel == "Ver Mapa General":
        ...
        if not display_df.empty:
            registrar_historial(
                calle_sel, display_df.iloc[0]["traffic_intensity"], display_df.iloc[0]["status"]
            )
    ...
    if calle_sel != "Ver Mapa General" and not display_df.empty:
        ...
        if info["status"] == "Rojo (Retención)":
            ...
            st.warning("⚠️ **AVISO:** Intensidad alta.")
        ...
        with c1:
            st.markdown(
                kpi_card("🚗", "Intensidad", int(intensidad), "v/h"), unsafe_allow_html=True
            )
        with c2:
            ...
            estado_limpio = (
                info["status"].split("(")[-1].replace(")", "")
                if "(" in info["status"]
                else info["status"]
            )
            st.markdown(
                kpi_card("🚥", "Estado", estado_limpio, "•", color_dot), unsafe_allow_html=True
            )
        with c3:
            st.markdown(
                kpi_card("⚡", "Límite IA", int(v_legal), "km/h", "#ff4b4b"), unsafe_allow_html=True
            )
        with c4:
            st.markdown(
                kpi_card("🛣️", "Capacidad", int(info.get("lanes", 0)), "Carriles", "#ffa500"),
                unsafe_allow_html=True,
            )
        ...
        with tab1:
            ...
            def get_trend_info(val_pred, val_actual):
                ...
                if diff > 100:
                    return "#ff4b4b", "📈", "Aumento Crítico"
                if diff > 0:
                    return "#ffa500", "↗️", "Incremento Ligero"
        ...
        with tab2:
            ...
            if len(hist_calle) > 1:
                ...
                with c1:
                    st.markdown(
                        stat_card("📉", "Mínimo", min_int, "#28a745"), unsafe_allow_html=True
                    )
                with c2:
                    st.markdown(
                        stat_card("📊", "Media", avg_int, "#007bff"), unsafe_allow_html=True
                    )
                with c3:
                    st.markdown(
                        stat_card("📈", "Máximo", max_int, "#ffc107"), unsafe_allow_html=True
                    )
        ...
        with tab3:
            ...
            ocupacion = (
                min(100, int((intensidad / capacidad_max) * 100)) if capacidad_max > 0 else 0
            )
